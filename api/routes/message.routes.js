...
router.get('/get/:id', authenticate, authorizeTeamRole(['admin', 'member']), messageController.getMessageById);
router.post('/start', authenticate, authorizeTeamRole(['admin', 'member']), restrictImpersonationActions, 
  uploadFiles('messages', 'files', 10, { useTeamFolders: true }), messageController.createMessage);
...
router.get('/search', authenticate, authorizeTeamRole(['admin', 'member']), messageController.searchMessages);
router.get('/conversations', authenticate, authorizeTeamRole(['admin', 'member']), messageController.getConversations);
router.get('/conversations/:type/:target_id/info', authenticate, authorizeTeamRole(['admin', 'member']), messageController.getConversationInfo);
...
router.get('/muted-chats', authenticate, authorizeTeamRole(['admin', 'member']), messageController.getMutedChats);
...
router.get('/files', authenticate, authorizeTeamRole(['admin', 'member']), messageController.getAllFilesFromConversation);
