...
import { setSelectedPinMessages } from '../../../../store/slices/chatSlice'
import { PinMessageSection, UserChannelOption } from '../../../../types'
...
const Pin = () => {
  ...
  const { selectedPinMessages } = useAppSelector((store) => store.chat)
  ...
  const {
    ...
    isFetchingNextPage,
    isLoading,
  } = queries.useGetSpecialMessagesInfinite({ ...chatParams, filter: 'pin' })
  ...
  useEffect(() => {
    if (messagesData?.pages) {
      ...
      dispatch(setSelectedPinMessages(processedMessages))
    }
  ...
  const defaultChatOption = useMemo(() => {
    ...
    return {
      ...
      data: {
        ...
        avatar: selectedChat.avatar ? selectedChat.avatar : null,
  ...
  return (
    <div className="pin-message custom-scrollbar">
      <UserChannelSelector
        name="groupMembers"
      ...
      />
      {selectedPinMessages.length === 0 ? (
        <div className="text-center p-4">
          <p className="text-muted">Pin the messages/files that are important for the channel.</p>
        </div>
      ) : (
        <>
          {(selectedPinMessages as PinMessageSection[]).map((item, index) =>
            item.messages.map((message, i: number) => <PinMessage selectedPinMessages={selectedPinMessages} key={`${index}-${i}`} message={message} />),
          )}
          {hasNextPage && (
